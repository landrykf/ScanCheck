...
export const LikeDislikes = (props) => {
  ...
  const onLike = () =>{
      if (likeAction === null){
          axios.post('/api/manga/upLike', variable)
            .then((response) => {
                if(response.data.success) {
                    setLikes(likes + 1)
                    setLikeAction('liked')
        ...
        axios.post('/api/manga/unLike', variable)
        .then((response) => {
            if(response.data.success) {
                setLikes(likes - 1)
                setLikeAction(null)
  ...
  return (
    <div>
      <span onClick = {onLike}>
          {likeAction === 'liked' ? (<img src="../icons/heart-filled.svg" alt="heart" />):(<img src="../icons/heart.svg" alt="heart" />) }
